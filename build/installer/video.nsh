;
; Filesmith setup, the picture: a video loop with the UI composited over it.
;
...
!define FRAMES 380         ; the whole loop, ~16 seconds at 24 fps
!define TICK   42
...
Function FilesmithPickOverlay
  ...
  ${ElseIf} $Screen = 2
    StrCpy $1 "copy"
  ${Else}
    ; the checkboxes are stamped at runtime, so one overlay serves every state
    StrCpy $1 "done"
  ${EndIf}
  ${If} $Hot != ""
    StrCpy $1 "$1-hot-$Hot"
